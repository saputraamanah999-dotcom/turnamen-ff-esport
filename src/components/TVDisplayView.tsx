...
export const TVDisplayView: React.FC<TVDisplayViewProps> = ({ scores, rounds, settings, teams, results, viewerMode = false, onExit }) => {
  ...
  const [streamMode, setStreamMode] = useState<'screen' | 'camera' | 'sample'>(viewerMode ? 'sample' : 'screen');
  const [isStreaming, setIsStreaming] = useState(viewerMode);
  ...
  const hasYoutubeVideo = youtubeVideoId.length === 11;
  ...
  // Auto-start demo video for viewer mode
  useEffect(() => {
    if (viewerMode) {
      setStreamMode('sample');
      setIsStreaming(true);
    }
    return () => stopGameplayStream();
  }, [viewerMode]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 z-10 my-auto flex-1 items-stretch">
        ...
        <div className={`flex flex-col relative rounded-2xl overflow-hidden border-2 border-slate-800 bg-slate-950/90 shadow-2xl min-h-[320px] ${layoutStyle === 'split' ? 'lg:col-span-8' : 'lg:col-span-12'}`}>
          ...
          <div className="relative w-full h-full flex items-center justify-center bg-black min-h-[300px] overflow-hidden">
            {streamMode === 'screen' || streamMode === 'camera' ? (
              ...
              <div className="relative w-full h-full min-h-[320px] flex items-center justify-center overflow-hidden">
                <iframe
                  ...
                  title="Tournament Gameplay"
                  className="w-full h-full min-h-[320px] pointer-events-none scale-125 object-cover"
              ...
              /* Animated Fallback - Free Fire themed background */
              <div className="relative w-full h-full min-h-[320px] flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-slate-950 via-orange-950/30 to-slate-950">
                ...
                <div className="absolute inset-0 overflow-hidden">
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-amber-500/5 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '0.5s' }} />
                </div>
                ...
                <div className="relative z-10 flex flex-col items-center text-center px-6">
                  ...
                  <h3 className="font-orbitron font-black text-xl md:text-2xl text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-orange-400 to-red-500 uppercase mb-3">
                    {tournamentName}
                  </h3>
                  <p className="text-sm text-slate-400 font-rajdhani font-bold max-w-md mb-2">
                    {liveStatus === 'live' ? 'Match sedang berlangsung...' : 'Menunggu dimulai...'}
                  </p>
                  {currentRoundLabel && (
                    <span className="px-3 py-1 rounded-lg bg-orange-950/60 border border-orange-500/40 font-orbitron font-bold text-xs text-orange-300 mb-4">
                      {currentRoundLabel}
                    </span>
                  )}
                  <p className="text-[11px] text-slate-500 font-rajdhani">
                    {hasYoutubeVideo ? 'Video tidak tersedia. Cek URL YouTube di Admin > Live Config.' : 'Set YouTube URL di Admin Panel > Live Match Config untuk tampilkan video gameplay.'}
                  </p>
                </div>
                {/* Scan line effect */}
            ...
            {!isStreaming && streamMode !== 'sample' && !viewerMode && (
            ...
            {/* Stream Controls (hidden in viewer mode) */}
            ...
            {!viewerMode && <button onClick={() => setIsMuted(!isMuted)} className="absolute bottom-3 right-3 z-30 p-2 rounded-lg bg-slate-900/80 border border-slate-700 text-slate-200 hover:bg-slate-800 transition-colors" title={isMuted ? 'Unmute' : 'Mute'}>
