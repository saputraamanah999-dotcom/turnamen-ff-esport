...
export const TVDisplayView: React.FC<TVDisplayViewProps> = ({ scores, rounds, settings, teams, results, viewerMode = false, onExit }) => {
  ...
  const AnimatedFallback = ({ status, message }: { status?: string; message?: string }) => (
    <div className="relative w-full h-full min-h-[320px] flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-slate-950 via-orange-950/30 to-slate-950">
  ...
  return (
    <div className="min-h-screen bg-[#050608] text-slate-100 flex flex-col justify-between p-3 md:p-4 lg:p-6 select-none relative overflow-hidden font-sans">
      ...
      <div className="relative z-20 space-y-3 mb-3">
        <div className="bg-slate-950/95 backdrop-blur-xl border border-slate-800/90 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-4 py-3 flex flex-col lg:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setLayoutStyle(layoutStyle === 'split' ? 'overlay' : 'split')} className="px-2.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 text-xs font-orbitron font-bold uppercase flex items-center gap-1" title="Toggle Layout"><Zap className="w-3.5 h-3.5 text-amber-400" /> {layoutStyle.toUpperCase()}</button>
              <button onClick={toggleFullscreen} className="p-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-200" title="Fullscreen">{isFullscreen ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}</button>
              {!viewerMode && onExit && <button onClick={onExit} className="px-3 py-2 rounded-xl bg-red-950/80 hover:bg-red-900 border border-red-600/50 text-red-300 font-orbitron font-bold text-xs uppercase">Keluar</button>}
            </div>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {currentRoundKillLeader && currentRoundKillLeader.result && currentRoundKillLeader.result.kill > 0 && (
            <motion.div key={currentRoundKillLeader.team.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ type: 'spring', stiffness: 300, damping: 25 }} className="bg-slate-950/90 border border-red-500/40 rounded-xl px-4 py-2 flex items-center justify-between gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 z-10 my-auto flex-1 items-stretch">
        <div className={`flex flex-col relative rounded-2xl overflow-hidden border-2 ${isLiveBroadcasting ? 'border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.3)]' : 'border-slate-800'} bg-slate-950/90 shadow-2xl min-h-[320px] ${layoutStyle === 'split' ? 'lg:col-span-8' : 'lg:col-span-12'}`}>
          {/* HUD */}
          ...
          <div className="relative w-full h-full flex items-center justify-center bg-black min-h-[300px] overflow-hidden">
              ...
              <>
                ...
                {isStreaming && (
                  <div className="absolute bottom-3 left-3 right-3 z-30 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <button onClick={() => startGameplayStream('screen')} className={`px-2.5 py-1.5 rounded-lg font-orbitron font-bold text-[10px] uppercase flex items-center gap-1 transition-all ${streamMode === 'screen' ? 'bg-orange-500 text-white' : 'bg-slate-900/80 text-slate-300'}`}><Monitor className="w-3 h-3" /> Screen</button>
                      <button onClick={() => startGameplayStream('camera')} className={`px-2.5 py-1.5 rounded-lg font-orbitron font-bold text-[10px] uppercase flex items-center gap-1 transition-all ${streamMode === 'camera' ? 'bg-orange-500 text-white' : 'bg-slate-900/80 text-slate-300'}`}><Camera className="w-3 h-3" /> Camera</button>
                      {/* GO LIVE BUTTON */}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setIsMuted(!isMuted)} className="p-1.5 rounded-lg bg-slate-900/80 border border-slate-700 text-slate-200 hover:bg-slate-800 transition-colors">{isMuted ? <VolumeX className="w-4 h-4 text-red-400" /> : <Volume2 className="w-4 h-4 text-emerald-400" />}</button>
                    </div>
                  </div>
                )}
              </>
            ...
            {viewerMode && liveRemoteStream && (
              <button onClick={() => setIsMuted(!isMuted)} className="absolute bottom-3 right-3 z-30 p-2.5 rounded-xl bg-slate-900/90 border border-slate-700 text-slate-200 hover:bg-slate-800 transition-colors" title={isMuted ? 'Unmute' : 'Mute'}>
