...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [liveYoutubeUrl, setLiveYoutubeUrl] = useState(settings.youtubeUrl || '');
  ...
  useEffect(() => {
    ...
    setLiveYoutubeUrl(settings.youtubeUrl || '');
  }, [settings]);
  ...
  const handleSaveLiveConfig = async (e: React.FormEvent) => {
    ...
    const newConfig: SettingsConfig = {
      ...
      status: liveStatus,
      youtubeUrl: liveYoutubeUrl.trim()
    };
    try {
      await updateSettings(newConfig);
      setLiveSuccess(true);
      ...
      {activeTab === 'livematch' && (
        <form onSubmit={handleSaveLiveConfig} className="bg-slate-950/90 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-2xl backdrop-blur-md space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            {/* YouTube Live / Gameplay URL */}
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2">
              <label className="block text-xs font-orbitron font-bold text-slate-200">YOUTUBE LIVE / GAMEPLAY URL</label>
              <input
                type="text"
                value={liveYoutubeUrl}
                onChange={(e) => setLiveYoutubeUrl(e.target.value)}
                placeholder="https://www.youtube.com/watch?v=VIDEO_ID atau ID video saja"
                className="w-full px-3.5 py-2 rounded-xl bg-slate-950 border border-slate-700 text-red-400 font-mono text-sm"
              />
              <span className="text-[11px] text-slate-500 block">Paste URL YouTube live/stream atau ID video. Akan tampil di mode Watch untuk penonton.</span>
            </div>
