...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const handleStartBroadcast = async () => {
    ...
    try {
      if (!navigator.mediaDevices?.getDisplayMedia) {
        throw new Error('Browser tidak mendukung tangkap layar. Gunakan Chrome/Edge di PC.');
      }
      ...
      // Jika user stop sharing dari browser bar
      stream.getVideoTracks()[0].onended = () => {
        handleStopBroadcast();
      };

      // Mulai WebRTC Broadcast
      ...
      // Update settings biar viewer tau status
  ...
  const handleStopBroadcast = async () => {
    ...
    try {
      await updateSettings({ ...settings, isBroadcasting: false } as SettingsConfig);
    } catch (e) { /* ignore */ }

    console.log('[Admin] Live broadcast stopped.');
  };
        ...
        <div className="bg-slate-950/90 border border-slate-800 rounded-2xl p-4 sm:p-6 shadow-2xl backdrop-blur-md space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2">
              ...
              <input type="text" value={liveTournamentName} onChange={(e) => setLiveTournamentName(e.target.value)} placeholder="FREE FIRE WORLD SERIES" className="w-full px-3.5 py-2 rounded-xl bg-slate-950 border border-slate-700 text-orange-400 font-orbitron font-bold text-lg" />
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2">
              ...
              <div className="flex gap-2">
                {(['waiting', 'live', 'finished'] as const).map((s) => (
                  <button key={s} type="button" onClick={() => setLiveStatus(s)} className={"flex-1 py-2.5 rounded-xl font-orbitron font-bold text-xs uppercase tracking-wider transition-all " + (liveStatus === s ? (s === 'live' ? 'bg-red-600 text-white shadow-lg shadow-red-600/50 animate-pulse' : s === 'finished' ? 'bg-slate-600 text-white' : 'bg-amber-600 text-white') : 'bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700')}>
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2">
              ...
              <select value={liveCurrentRoundId} onChange={(e) => setLiveCurrentRoundId(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-orange-500/40 text-orange-400 font-orbitron font-bold text-sm focus:outline-none">
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2">
              ...
              <input type="text" value={liveBannerUrl} onChange={(e) => setLiveBannerUrl(e.target.value)} placeholder="https://example.com/banner.jpg" className="w-full px-3.5 py-2 rounded-xl bg-slate-950 border border-slate-700 text-amber-400 font-mono text-sm" />
              {liveBannerUrl && (
          ...
          <div className={"p-5 rounded-2xl border-2 " + (isBroadcasting ? "bg-red-950/40 border-red-500/60" : "bg-slate-900/60 border-slate-800")}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Monitor className="w-5 h-5 text-orange-400" />
                <span className="font-orbitron font-bold text-sm text-slate-200">LIVE SCREEN BROADCAST (WebRTC)</span>
              </div>
              {isBroadcasting && (
                <span className="px-2.5 py-0.5 rounded-full bg-red-600 text-white font-orbitron font-black text-[10px] uppercase animate-pulse flex items-center gap-1.5">
            ...
            {isBroadcasting ? (
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-red-950/50 border border-red-500/30">
                  <div className="w-3 h-3 rounded-full bg-red-500 animate-ping" />
                  <span className="text-sm font-orbitron font-bold text-red-300">SEDANG LIVE - Layar kamu lagi di-broadcast ke semua penonton</span>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900 border border-slate-700">
                    <Wifi className="w-4 h-4 text-emerald-400" />
                    <span className="font-orbitron font-bold text-xs text-slate-200">Penonton: <strong className="text-emerald-400">{broadcastViewerCount}</strong></span>
                  </div>
                  <button type="button" onClick={handleStopBroadcast} className="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-orbitron font-bold text-xs uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-red-600/30 transition-all">
                    <MonitorOff className="w-4 h-4" /> STOP BROADCAST
                  </button>
                </div>
                <span className="text-[11px] text-slate-500 block">Klik STOP atau hentikan share layar dari browser bar untuk menghentikan broadcast.</span>
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-xs text-slate-400 font-rajdhani font-semibold">Tangkap layar komputer kamu dan broadcast langsung ke semua penonton di <strong className="text-orange-400">#watch</strong> tanpa YouTube. Pakai WebRTC peer-to-peer langsung dari browser.</p>
                <button type="button" onClick={handleStartBroadcast} className="px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-400 hover:to-green-500 text-white font-orbitron font-bold text-sm uppercase tracking-wider flex items-center gap-2.5 shadow-lg shadow-emerald-600/30 transition-all">
                  <Monitor className="w-5 h-5" /> MULAI LIVE BROADCAST
                </button>
                {broadcastError && (
                  <div className="p-2.5 rounded-lg bg-red-950/80 border border-red-500/40 text-red-300 text-xs font-rajdhani font-bold flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4 shrink-0" /> {broadcastError}
                  </div>
                )}
                <span className="text-[11px] text-slate-500 block">Pastikan pakai <strong>Chrome/Edge di PC</strong>. Layar Free Fire yang ditangkap akan langsung dilihat penonton.</span>
              </div>
            )}
          </div>

          <div className="pt-4 border-t border-slate-800 flex justify-end">
            <button type="button" onClick={(e: any) => handleSaveLiveConfig(e)} className="px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-red-600 font-orbitron font-bold text-xs text-white uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-orange-500/20">
