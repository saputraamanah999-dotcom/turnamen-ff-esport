...
// ====== ICE SERVERS (STUN + Free TURN for cross-device) ======
const ICE_SERVERS: RTCConfiguration = {
  iceServers: [
    // Google STUN
    { urls: 'stun:stun.l.google.com:19302' },
    { urls: 'stun:stun1.l.google.com:19302' },
    // Free TURN relay (metered.ca) - biar bisa beda device/wifi
    {
      urls: 'turn:a.relay.metered.ca:80',
      username: 'e8dded9d6b8c2f2f5f49a505',
      credential: 'vMw+3KvM3r0Xz7YJ'
    },
    {
...
export class Broadcaster {
  ...
  async start(mediaStream: MediaStream): Promise<void> {
    ...
    // Clean up stale viewer docs from previous sessions
    try {
    ...
    const unsubViewers = onSnapshot(viewersPath(), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added' && !this.processedViewers.has(change.doc.id)) {
          ...
          setTimeout(() => this.connectToViewer(change.doc.id), 500);
...
export class Viewer {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (!statusDoc.exists() || !statusDoc.data()?.active) {
        this.onStatusCallback('waiting', 'Stream belum dimulai. Tunggu admin mulai live.');
  ...
  private async establishConnection(): Promise<void> {
    ...
    this.pc.ontrack = (event) => {
      console.log('[Viewer] Received remote track:', event.track.kind, 'streams:', event.streams.length);
      if (event.streams[0]) {
    ...
    this.pc.onconnectionstatechange = () => {
      ...
      if (state === 'connected') {
        ...
        this._isConnected = false;
        this.onStatusCallback('error', 'Koneksi gagal. Mencoba reconnect otomatis...');
        // Auto reconnect after 5s
        if (!this.destroyed) {
          this.reconnectTimeout = setTimeout(() => {
            if (!this.destroyed && !this._isConnected) {
              console.log('[Viewer] Attempting reconnect after failure...');
              this.cleanupCurrentConnection();
              // Re-register with new viewer ID
              this.viewerId = 'v_' + Date.now().toString(36) + '_' + Math.random().toString(36).substr(2, 6);
              this.establishConnection();
            }
          }, 5000);
    ...
    const unsubOffer = onSnapshot(offerPath(this.viewerId), async (docSnap) => {
      if (docSnap.exists() && this.pc && !this.destroyed) {
    ...
    const unsubIceAdmin = onSnapshot(iceAdminPath(this.viewerId), (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added' && !processedIce.has(change.doc.id) && this.pc && !this.destroyed) {
          processedIce.add(change.doc.id);
          const data = change.doc.data();
  ...
  private cleanupCurrentConnection(): void {
    ...
    this.cleanups.forEach(fn => { try { fn(); } catch (e) { /* ignore */ } });
    this.cleanups = [];
    // Remove old viewer registration
