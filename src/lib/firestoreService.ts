  ...
  try {
    ...
    roundDocs.forEach((rDoc, rIdx) => {
      teamDocs.forEach((tDoc, tIdx) => {
        ...
        const teamRes = matchR ? matchR[tIdx] : { kill: 3, placement: tIdx + 1, booyah: tIdx === 0 };
...
export function getRoundWinner(
  ...
  // Find the team with placement = 1 (booyah winner)
...
export async function deleteTeam(id: string) {
  // Also delete all results for this team
  const resultsSnap = await getDocs(resultsRef);
  const batch = writeBatch(db);
...
export async function deleteRound(id: string) {
  // Also delete all results for this round
  const resultsSnap = await getDocs(resultsRef);
  const batch = writeBatch(db);
  resultsSnap.forEach((docSnap) => {
    const data = docSnap.data();
